...
export const getMovies = () => {
    return fetch(
      `https://api.themoviedb.org/3/discover/movie?api_key=${process.env.REACT_APP_TMDB_KEY}&language=en-US&primary_release_year=...&certification=....&include_video=false&page=2`
    ).then((response) => {
      if (!response.ok) {
        throw new Error(response.json().message);
      }
